(function(){
    if (typeof define === 'function' && define.amd) {
        define('Calender', [], function() {
            return Calender;
        });
    } else if (typeof exports !== 'undefined') {
        if (typeof module !== 'undefined' && module.exports) {
            exports = module.exports = Calender;
        }
...
var m = Math,dummyStyle = doc.createElement('div').style,
	...
	Calender.prototype={
		...
		init:function(){
			...
			if(!calender){
				...
				if(that.options.more){
					calenderArr.push('<button id="cal_more" class="cal_more" type="button">'+that.options.moreName+'</botton>');
				}
			...
			if(typeof seajs!=="undefined"){
				...
				style.setAttribute("href",that.options.style+".css");
				//<link charset="utf-8" rel="stylesheet" href="file:///D:/projects/Team777/team-front/mobileDate/default.css">
				header.appendChild(style);
			}
			
			if(that.options.init){
				that.options.init.call(that,that.dateFormat(that.options.format,new Date()),this);
			}
			//slider.width=""
						...
						for(var j=0,jLen=oTds.length;j<jLen;j++){
							...
							if(oTds[j].className.indexOf("disable")==-1){
								oTds[j].onclick=function(){
									...
									date.setDate(day);
									var formatDate=parseInt(that.dateFormat(that.options.format,date),10);
									//执行启程函数
									if(that.options.startShow){
		...
		nextMonth:function(calender){
			...
			var that=this;
			that.reloadCalender(calender,that.options.year,that.options.month+1);
			var unit=that.unit;			
		...
		prevMonth:function(calender){
			//$(calender.find(".cal_show")).fadeOut("slow");
			var that=this;
			that.reloadCalender(calender,that.options.year,that.options.month-that.options.monthCount-1);
			var unit=that.unit;	
			var moveBy=that.moveBy;					
			var browserWidth=that.browserWidth;		
			calender.style[unit]=browserWidth+"px";
